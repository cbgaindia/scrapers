...
class UnionBudgetsScrapper2003_10(ScrappingUtils):
            ...
            if ".htm" in child_url:
                ...
                if not out_links:
                    ...
                    if not self.save_file_as_xls(page_text, file_path):
                        ...
                        self.save_file_as_txt(page_text, file_path, xpath=TEXT_DOC_XPATH)
            else:
    ...
    def get_file_name_for_link(self, link):
        ...
        file_language = None
        file_name_xpath = ".//preceding::td[1]//text()"
        while not file_name_found:
            language_tag_found = False
            for language_tag in LANGUAGE_TAGS:
                if language_tag in file_name:
                    file_name = self.get_text_from_element(link, xpath=file_name_xpath)
                    file_name_xpath = "./." + file_name_xpath
